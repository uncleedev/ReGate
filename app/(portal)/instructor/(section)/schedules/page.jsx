...
export default function InstructorSchedulePage() {
  ...
  useEffect(() => {
    ...
    const fetchInstructor = async () => {
      try {
        const res = await fetch("http://localhost:3003/instructors", {
          method: "GET",
          headers: {
            "Content-Type": "application/json"
          }
        ...
        const data = await res.json();
        const foundInstructor = data.find(item => item.email === email);
  ...
  useEffect(() => {
    if (schedules.length > 0 && instructor) {
      const mySchedules = schedules.filter(schedule => 
        schedule.instructor_name === `${instructor.firstname} ${instructor.lastname}`
      );
