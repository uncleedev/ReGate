...
export default function AnnouncementSection() {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setAnnouncements((prev) => prev.filter((item) => item._id !== currentItem._id));
        setShowDeleteModal(false); 
      } else {
        const errorData = await res.json();
        setError(errorData.message || "Failed to delete the announcement");
      }
    } catch (error) {
  ...
  const handleSubmitCreate = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setAnnouncements((prev) => [...prev, announcement.announcement]);
  ...
  const handleSubmitEdit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const errorData = await res.json();
        setError(errorData.message || "Failed to update the announcement");
      }
    } catch (error) {
  ...
  <>
          ...
          <table className="flex flex-col w-full">
            <thead className="w-full">
              <tr className={`flex justify-between items-center p-2 bg-[#044721] bg-opacity-75 text-white rounded-t-md`}>
                ...
                <th className="data items-center">Headline</th>
                <th className="data items-center">Caption</th>
    ...
    <Modal isOpen={showCreateModal} onClose={handleCloseCreateModal} title={"Create new Announcement"}>
      ...
      <Modal isOpen={showEditModal} onClose={handleCloseEditModal} title={"Edit Announcement"}>
